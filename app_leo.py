...
# Load the model:
model = models.load_model('latest_trained_model.h5')
model2 = models.load_model('ad_detection_model.h5')
model3 = models.load_model('model.h5')
...
if podcast_file is not None:
    ...
    if st.button("🚫 Detect and Remove Ads"):
        ...
        ad_segments = detect_ads(tmp_file_path, model3, clip_duration=5) # Detect ads using the model
 ...
 # Display two options for playing the processed podcasts
    ...
    if st.button("🎶 Play Podcast Without Ads"):
        st.audio(clean_podcast_path, format='audio/mp3')

    if st.button("🔊 Play Ads Only"):
        st.audio(ads_only_path, format='audio/mp3')
